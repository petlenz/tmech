...
static void double_contraction_4_2_simd(benchmark::State& state) {
    ...
    for (std::size_t i = 0; i < N * N; ++i) A[i] = static_cast<T>(i) * T(0.01);
    for (std::size_t i = 0; i < N; ++i)     x[i] = static_cast<T>(i) * T(0.1);
...
////template<typename T, std::size_t SIZE, std::size_t Dim>
////static void BM_general_double_contraction_array_vector(benchmark::State& state) {

////    using seqL = tmech::sequence<2>;
////    using seqR = tmech::sequence<1>;

////    std::vector<std::array<std::array<std::array<std::array<T,Dim>,Dim>,Dim>,Dim>> A(SIZE);
////    std::vector<std::array<std::array<std::array<std::array<T,Dim>,Dim>,Dim>,Dim>> a(SIZE), res(SIZE);
////    //std::vector<T[Dim][Dim][Dim][Dim]> A(SIZE);
////    //std::vector<T[Dim][Dim][Dim][Dim]> a(SIZE), res(SIZE);
...
////template<typename T, std::size_t SIZE, std::size_t Dim>
////static void BM_second_general_double_contraction_array_vector(benchmark::State& state) {

////    using seqL = tmech::sequence<2>;
////    using seqR = tmech::sequence<1>;

////    std::vector<std::array<std::array<T,Dim>,Dim>> A(SIZE);
////    std::vector<std::array<std::array<T,Dim>,Dim>> a(SIZE), res(SIZE);
////    //std::vector<T[Dim][Dim][Dim][Dim]> A(SIZE);
////    //std::vector<T[Dim][Dim][Dim][Dim]> a(SIZE), res(SIZE);
////    for(int ii{0}; ii<SIZE; ++ii)
////    for(std::size_t i{0}; i<Dim; ++i){
