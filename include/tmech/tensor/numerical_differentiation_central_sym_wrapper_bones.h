...
struct set_basis
{
    ...
    static constexpr inline auto set(Tensor & basis, Tuple const& tuple, T const value){
        tuple_call(basis, tuple, min_value_squence_t<First,1>()) = value;
        set_basis<Args...>::set(basis, tuple, value);
    }

    template<typename Tensor, typename T, typename Tuple>
    static constexpr inline auto add(Tensor & basis, Tuple const& tuple, T const value){
        tuple_call(basis, tuple, min_value_squence_t<First, 1>()) += value;
...
/**
* @class numerical_differentiation_backward_wrapper
