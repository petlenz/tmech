...
//
// Tests that involve numerically sensitive operations (inversion,
// determinant, eigendecomposition, polar decomposition, numerical
// differentiation) must NOT use randn() because:
//   - Random matrices can be near-singular → inv() blows up
//   - Random symmetric matrices can have near-repeated eigenvalues
//   - Tolerances that pass 99% of the time still cause CI flakiness
//
// Instead, use these helpers which produce well-conditioned matrices
// with a deterministic seed derived from compile-time constants.
//==========================================================================

namespace test_helpers {

/// Seed a std::mt19937 from compile-time type/dim/rank info
/// so each test instantiation gets a unique but reproducible sequence.
template <typename T, std::size_t Dim, std::size_t Rank>
inline std::mt19937 &get_rng() {
  // Combine Dim, Rank, and sizeof(T) into a unique seed
  static std::mt19937 gen(42u + Dim * 1000u + Rank * 100u + sizeof(T) * 10u);
  return gen;
}

/// Fill a tensor with deterministic pseudo-random values in [-1, 1]
template <typename T, std::size_t Dim, std::size_t Rank>
inline auto seeded_randn(tmech::tensor<T, Dim, Rank> &t) {
  auto &gen = get_rng<T, Dim, Rank>();
  std::uniform_real_distribution<T> dist(static_cast<T>(-1), static_cast<T>(1));
  constexpr std::size_t N = tmech::tensor<T, Dim, Rank>::size();
  for (std::size_t i = 0; i < N; ++i) {
    t.raw_data()[i] = dist(gen);
  }
}

/// Create a well-conditioned rank-2 tensor (condition number ~ 10)
/// by constructing A = Q * D * Q^T where D has controlled eigenvalues.
template <typename T, std::size_t Dim> inline auto well_conditioned_rank2() {
  tmech::tensor<T, Dim, 2> A;
  // Use a fixed, known well-conditioned matrix
  if constexpr (Dim == 2) {
    // Eigenvalues: 1, 3  →  cond = 3
    A = tmech::tensor<T, 2, 2>{static_cast<T>(2.0), static_cast<T>(1.0),
                               static_cast<T>(1.0), static_cast<T>(2.0)};
  } else if constexpr (Dim == 3) {
    // Eigenvalues: ~0.24, ~1.26, ~4.34  →  cond ~ 18
    A = tmech::tensor<T, 3, 2>{
        static_cast<T>(2.0306), static_cast<T>(0.162), static_cast<T>(2.0208),
        static_cast<T>(0.162),  static_cast<T>(1.245), static_cast<T>(0.0),
        static_cast<T>(2.0208), static_cast<T>(0.0),   static_cast<T>(2.561)};
  }
  return A;
}

/// Create a well-conditioned non-symmetric rank-2 tensor.
/// Constructed as (well_conditioned_spd + skew) so it's invertible.
...
/// Create a well-conditioned SPD rank-2 tensor (for eigenvalue/sign/sqrt tests)
template <typename T, std::size_t Dim>
inline auto well_conditioned_spd_rank2() {
  return well_conditioned_rank2<T, Dim>();
}

/// Create a second well-conditioned non-symmetric rank-2 tensor (for tests
/// needing two distinct matrices)
template <typename T, std::size_t Dim>
inline auto well_conditioned_nonsym_rank2_b() {
  tmech::tensor<T, Dim, 2> B;
  if constexpr (Dim == 2) {
    B = tmech::tensor<T, 2, 2>{static_cast<T>(2.0), static_cast<T>(0.7),
                               static_cast<T>(-0.3), static_cast<T>(3.5)};
  } else if constexpr (Dim == 3) {
    B = tmech::tensor<T, 3, 2>{
        static_cast<T>(4.0),  static_cast<T>(0.3),  static_cast<T>(-0.7),
        static_cast<T>(-0.5), static_cast<T>(2.0),  static_cast<T>(0.8),
        static_cast<T>(0.4),  static_cast<T>(-0.6), static_cast<T>(3.0)};
  }
  return B;
}

/// Create a well-conditioned deformation-gradient-like tensor F
/// (for polar decomposition tests: det(F) > 0, well-conditioned)
template <typename T, std::size_t Dim> inline auto well_conditioned_defgrad() {
  tmech::tensor<T, Dim, 2> F;
  if constexpr (Dim == 2) {
    F = tmech::tensor<T, 2, 2>{static_cast<T>(1.2), static_cast<T>(0.3),
                               static_cast<T>(-0.1), static_cast<T>(1.5)};
  } else if constexpr (Dim == 3) {
    F = tmech::tensor<T, 3, 2>{
        static_cast<T>(1.3),   static_cast<T>(0.2),   static_cast<T>(-0.1),
        static_cast<T>(-0.15), static_cast<T>(1.1),   static_cast<T>(0.25),
        static_cast<T>(0.1),   static_cast<T>(-0.05), static_cast<T>(1.4)};
  }
  return F;
}
...
#define compareEqualOperatorFunction(ValueType, Dim, Rank, Function)            \
  TEST(                                                                         \
      gtest,                                                                    \
      compareEqualOperatorFunction_##ValueType##_##Dim##_##Rank##_##Function) { \
    tmech::tensor<ValueType, Dim, Rank> a, b;                                   \
    a.randn();                                                                  \
    b = a;                                                                      \
...
#define compareGreaterEqualOperatorEvaluate(ValueType, Dim, Rank)              \
  TEST(gtest,                                                                  \
       compareGreaterEqualOperatorEvaluate_##ValueType##_##Dim##_##Rank) {     \
    tmech::tensor<ValueType, Dim, Rank> a, b;                                  \
    a.fill(static_cast<ValueType>(2));                                         \
    b.fill(static_cast<ValueType>(1));                                         \
    EXPECT_EQ(true, (a + a >= b * 2));                                         \
...
// inv full rank 4 — STABILIZED: fixed well-conditioned rank-4 tensor
...
// cofactor function — STABILIZED: fixed matrix
...
// basis change second order — safe with randn (no inversion)
...
// trace of a second oder tensor — safe with randn
...
#define DoubleContractionLeft(ValueType, Dim, RankLHS, RankRHS)                \
  TEST(gtest,                                                                  \
       DoubleContractionLeft_##ValueType##_##Dim##_##RankLHS##_##RankRHS) {    \
...
#define DoubleContractionRight(ValueType, Dim, RankLHS, RankRHS)               \
  TEST(gtest,                                                                  \
       DoubleContractionRight_##ValueType##_##Dim##_##RankLHS##_##RankRHS) {   \
    tmech::tensor<ValueType, Dim, RankLHS> B, C1, C2;                          \
    tmech::tensor<ValueType, Dim, RankRHS> A;                                  \
    A.randn();                                                                 \
    B.randn();                                                                 \
...
#define derivatives_polar_decomposition_eigen(ValueType, Dim)                  \
  TEST(gtest, derivatives_polar_decomposition_eigen_##ValueType##_##Dim) {     \
    auto F = test_helpers::well_conditioned_defgrad<ValueType, Dim>();         \
    auto Fpolar = tmech::polar_decomposition(F);                               \
    auto lambda_R = [](auto const &t) {                                        \
      return tmech::eval(tmech::polar_decomposition(t).R());                   \
    };                                                                         \
    auto lambda_U = [](auto const &t) {                                        \
      return tmech::eval(tmech::polar_decomposition(t).U());                   \
    };                                                                         \
    auto lambda_V = [](auto const &t) {                                        \
      return tmech::eval(tmech::polar_decomposition(t).V());                   \
    };                                                                         \
    auto F_R_numdif =                                                          \
        tmech::num_diff_central<tmech::sequence<1, 2, 3, 4>>(lambda_R, F);     \
    auto F_U_numdif =                                                          \
        tmech::num_diff_central<tmech::sequence<1, 2, 3, 4>>(lambda_U, F);     \
    auto F_V_numdif =                                                          \
...
#define derivatives_polar_decomposition_newton(ValueType, Dim)                 \
  TEST(gtest, derivatives_polar_decomposition_newton_##ValueType##_##Dim) {    \
    ...
    auto lambda_R = [](auto const &t) {                                        \
      return tmech::eval(tmech::polar_decomposition(t, true, 5e-12, 50).R());  \
    };                                                                         \
    auto lambda_U = [](auto const &t) {                                        \
      return tmech::eval(tmech::polar_decomposition(t, true, 5e-12, 50).U());  \
    };                                                                         \
    auto lambda_V = [](auto const &t) {                                        \
      return tmech::eval(tmech::polar_decomposition(t, true, 5e-12, 50).V());  \
    };                                                                         \
    auto F_R_numdif =                                                          \
        tmech::num_diff_central<tmech::sequence<1, 2, 3, 4>>(lambda_R, F);     \
    auto F_U_numdif =                                                          \
...
//==========================================================================
// Symdiff scalar tests — deterministic (no randn)
//==========================================================================
...
//==========================================================================
// Symdiff tensor tests — STABILIZED: fixed matrices for sensitive operations
//==========================================================================
...
#define productRuleTensor(ValueType, Dim, Rank)                                \
  TEST(gtest, productRuleTensor_##ValueType##_##Dim##_##Rank) {                \
    ...
    EXPECT_EQ(                                                                 \
        df(f_var),                                                             \
        (tmech::basis_change<tmech::sequence<1, 3, 4, 2>>(                     \
             tmech::inner_product<tmech::sequence<2>, tmech::sequence<1>>(     \
                 tmech::otimesu(I, I), x(f_var))) +                            \
         tmech::inner_product<tmech::sequence<2>, tmech::sequence<1>>(         \
             x(f_var), tmech::otimesu(I, I))));                                \
  }
...
#define lambdaTensorSym_3(ValueType, Rank, Lambda)                             \
  TEST(gtest, lambdaTensorSym_3_##ValueType##_##Rank##_##Lambda) {             \
    ...
    tmech::tensor<ValueType, 3, Rank> a{2.0306, 0.162,  2.0208, 0.162, 1.245,  \
                                        0,      2.0208, 0,      2.561};        \
    auto f{tmech::Lambda(x)};                                                  \
    auto f_var{std::make_tuple(a)};                                            \
    auto df = symdiff::derivative<1>(f, x);                                    \
...
//==========================================================================
// Adaptor tests (all use fixed data — already stable)
//==========================================================================
...
TEST(gtest, adaptorVoigt2Dfourth) {
  double ptr[9]{1111, 1122, 1112, 2211, 2222, 2212, 1211, 1222, 1212};
...
TEST(gtest, adaptorVoigt3Dfourth) {
  double ptr[36]{1111, 1122, 1133, 1123, 1113, 1112, 2211, 2222, 2233,
                 2223, 2213, 2212, 3311, 3322, 3333, 3323, 3313, 3312,
                 2311, 2322, 2333, 2323, 2313, 2312, 1311, 1322, 1333,
                 1323, 1313, 1312, 1211, 1222, 1233, 1223, 1213, 1212};
  tmech::adaptor<double, 3, 4, tmech::voigt<3>> a(ptr);
  tmech::tensor<double, 3, 4> b{
      1111, 1112, 1113, 1112, 1122, 1123, 1113, 1123, 1133, 1211, 1212, 1213,
      1212, 1222, 1223, 1213, 1223, 1233, 1311, 1312, 1313, 1312, 1322, 1323,
      1313, 1323, 1333, 1211, 1212, 1213, 1212, 1222, 1223, 1213, 1223, 1233,
      2211, 2212, 2213, 2212, 2222, 2223, 2213, 2223, 2233, 2311, 2312, 2313,
      2312, 2322, 2323, 2313, 2323, 2333, 1311, 1312, 1313, 1312, 1322, 1323,
...
TEST(gtest, adaptorFull2Dfourth) {
  double ptr[16]{1111, 1112, 1121, 1122, 1211, 1212, 1221, 1222,
...
TEST(gtest, adaptorFull3Dfourth) {
  double ptr[81]{
      1111, 1112, 1113, 1121, 1122, 1123, 1131, 1132, 1133, 1211, 1212, 1213,
      1221, 1222, 1223, 1231, 1232, 1233, 1311, 1312, 1313, 1321, 1322, 1323,
      1331, 1332, 1333, 2111, 2112, 2113, 2121, 2122, 2123, 2131, 2132, 2133,
      2211, 2212, 2213, 2221, 2222, 2223, 2231, 2232, 2233, 2311, 2312, 2313,
      2321, 2322, 2323, 2331, 2332, 2333, 3111, 3112, 3113, 3121, 3122, 3123,
...
//==========================================================================
// Standalone tests — STABILIZED where needed
//==========================================================================
...
TEST(gtest, exponential_map) {
  ...
  tmech::tensor<double, 3, 2> Aexp{
      2.333496383103521e+00, -9.424245337660712e-01, 6.174107765089896e-01,
...
// symdiff numdiff tests — STABILIZED: fixed matrices
...
TEST(gtest, symdiff_numdiff_tensor_norm) {
  symdiff::variable<tmech::tensor<double, 3, 2>, 0> Avar;
  auto A = test_helpers::well_conditioned_nonsym_rank2<double, 3>();
  auto func = [](auto const &tensor) {
    return std::sqrt(tmech::dcontract(tensor, tensor));
  };
  auto s_func = std::sqrt(
      tmech::inner_product<tmech::sequence<1, 2>, tmech::sequence<1, 2>>(Avar,
                                                                         Avar));
  auto dA_num = tmech::num_diff_central(func, A);
...
TEST(gtest, symdiff_numdiff_tensor_dcontract_2_2) {
  ...
  auto A = test_helpers::well_conditioned_nonsym_rank2<double, 3>();
  auto func = [](auto const &tensor) {
    return tmech::dcontract(tensor, tensor);
  };
...
lambdaTensorSym_3(double, 2, dev);
