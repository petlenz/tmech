...
TEST(gtest, inv_full_function_4_##ValueType##_##Dim##_){ \
    ...
    const auto Anew{tmech::basis_change<tmech::sequence<1,3,4,2>>(A)}; \
    EXPECT_EQ(true, tmech::almost_equal(tmech::dcontract(tmech::invf(A),A), II, 5e-4)); \
    EXPECT_EQ(true, tmech::almost_equal(tmech::dcontract(tmech::invf(tmech::abs(A)),tmech::abs(A)), II, 5e-4)); \
    EXPECT_EQ(true, tmech::almost_equal(tmech::dcontract(tmech::basis_change<tmech::sequence<1,4,2,3>>(tmech::invf<tmech::sequence<1,4,2,3>>(Anew)),A), II, 5e-4)); \
    EXPECT_EQ(true, tmech::almost_equal(tmech::dcontract(tmech::basis_change<tmech::sequence<1,4,2,3>>(tmech::invf<tmech::sequence<1,4,2,3>>(tmech::abs(Anew))),tmech::abs(A)), II, 5e-4)); \
}
...
struct polar_eigen_R
{
    template<typename _Tensor>
    constexpr inline _Tensor operator()(_Tensor const& __tensor)const{
        return tmech::polar_decomposition(__tensor).R();
    }
};


struct polar_eigen_U
{
    template<typename _Tensor>
    constexpr inline _Tensor operator()(_Tensor const& __tensor)const{
        return tmech::polar_decomposition(__tensor).U();
    }
};


struct polar_eigen_V
{
    template<typename _Tensor>
    constexpr inline _Tensor operator()(_Tensor const& __tensor)const{
        return tmech::polar_decomposition(__tensor).V();
    }
};

struct polar_newton_R
{
    template<typename _Tensor>
    constexpr inline _Tensor operator()(_Tensor const& __tensor)const{
        return tmech::polar_decomposition(__tensor, true, 5e-12, 50).R();
    }
};


struct polar_newton_U
{
    template<typename _Tensor>
    constexpr inline _Tensor operator()(_Tensor const& __tensor)const{
        return tmech::polar_decomposition(__tensor, true, 5e-12, 50).U();
    }
};


struct polar_newton_V
{
    template<typename _Tensor>
    constexpr inline _Tensor operator()(_Tensor const& __tensor)const{
        return tmech::polar_decomposition(__tensor, true, 5e-12, 50).V();
    }
};
...
TEST(gtest, derivatives_polar_decomposition_eigen_##ValueType##_##Dim){ \
    ...
    auto F_R_numdif = tmech::num_diff_central<tmech::sequence<1,2,3,4>>(polar_eigen_R(), F); \
    auto F_U_numdif = tmech::num_diff_central<tmech::sequence<1,2,3,4>>(polar_eigen_U(), F); \
    auto F_V_numdif = tmech::num_diff_central<tmech::sequence<1,2,3,4>>(polar_eigen_V(), F); \
...
TEST(gtest, derivatives_polar_decomposition_newton_##ValueType##_##Dim){ \
    ...
    auto F_R_numdif = tmech::num_diff_central<tmech::sequence<1,2,3,4>>(polar_newton_R(), F); \
    auto F_U_numdif = tmech::num_diff_central<tmech::sequence<1,2,3,4>>(polar_newton_U(), F); \
    auto F_V_numdif = tmech::num_diff_central<tmech::sequence<1,2,3,4>>(polar_newton_V(), F); \
    EXPECT_EQ(true, tmech::almost_equal(Fpolar.R().derivative(), F_R_numdif, 5e-5)); \
    EXPECT_EQ(true, tmech::almost_equal(Fpolar.V().derivative(), F_V_numdif, 5e-5)); \
    EXPECT_EQ(true, tmech::almost_equal(Fpolar.U().derivative(), F_U_numdif, 5e-5)); \
}
...
TEST(gtest, symdiff_numdiff_tensor_inv){
    ...
    auto dA_ana = symdiff::derivative<1>(s_func, Avar);
    std::cout<<dA_ana(std::make_tuple(A))<<std::endl;
    EXPECT_EQ(true, tmech::almost_equal(dA_num, dA_ana(std::make_tuple(A)), 5e-6));
}
